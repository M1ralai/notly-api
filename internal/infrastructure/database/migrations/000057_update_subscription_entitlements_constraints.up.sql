...
ALTER TABLE subscription_entitlements
  ADD CONSTRAINT subscription_entitlements_provider_check
    CHECK (provider IN ('apple', 'google', 'revenuecat', 'stripe', 'rc_billing', 'promotional', 'admin')),
  ADD CONSTRAINT subscription_entitlements_plan_check
    CHECK (plan IN ('monthly', 'yearly', 'annual', 'lifetime', 'free', 'custom')),
  ADD CONSTRAINT subscription_entitlements_status_check
    CHECK (status IN ('active', 'expired', 'cancelled', 'revoked', 'grace_period', 'billing_issue', 'in_trial'));
